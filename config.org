...
The add torrent function of transmission (the Emacs package) only works for files. This makes it so that it only works for magnets instead.
This replaces the
#+begin_src elisp
;(defun olav-torrent ()
;  (interactive)
...
#+end_src
* Scratch Perspective
I use a scratch perspective to do tasks that don't fit in the current perspective or any other project. For instance if I am to install a global package when I am in the irc perspective I would switch to the scratch perspective an do it there.
#+begin_src elisp
(defun olav-scratch ()
  (interactive)
